...
class fsAPI
{
	...
	*/
	...
	{
//echo (BASE_URL);
		$url = FatSecretAPI::$base . 'method=foods.search&search_expression=salt';//&format=json';
//		$url = FatSecretAPI::BASE_URL . 'method=foods.search&search_expression=salt';//&format=json';
...
foreach ($doc AS $id => $food)
{
var_dump($food);
echo "$id = ".$food->food_name."<br />";
//    printf("%s has got %d children.\n", $food['name'], $food->count());
}
		$this->ErrorCheck($doc);

//var_dump("count=".$doc->count());
	...
	*/
	...
	private function ErrorCheck($doc)
	{
		...
		{
			show_error($doc->message, (int)$doc->code);
