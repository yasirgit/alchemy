		...
		<tr>
			<td class="leftCell">
				<div class="leftCellContent">
					<form id="recipeform"  enctype="multipart/form-data" method="post" action="recipes/recipeSubmit">
					<input type="hidden" name="rID" value="<?=@$recipe[0]->rID?>" />
						...
						<div style="width:100%;display:table">
							...
							<div style="margin:10px;">
								...
								if (@$recipeTypes)
								{
									foreach ($recipeTypes AS $rType)
									{
										if (@$rType->name)
										{
								?>			
										<div style="float:left; width:190px;"><input type="checkbox" name="recipe_types[]" value="<?php echo $rType->id;?>" ><?php echo $rType->name;?></div>
								<?php
										}
									}
								}
					...
					<br/>
					<div>
						Recipe Image: <input type="file" name="recipe_image"/>
						<div id="divNutrition"></div>
					</div>
					<div align="right">
						<input type="submit"	id="saveRecipe"		value="Save Recipe"	name="saveRecipe" />
						<!--input type="button"	id="saveRecipe"	value="Save Recipe" name="saveRecipe" onClick="$('#recipeform').ajaxSubmit({target: '#recipeform',url: 'recipes/recipeSubmit'}); return false;"/-->
						or
