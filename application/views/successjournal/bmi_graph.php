...
foreach($graph_week_val as $gval)
 {
	...
	$getweek[$i] = date('m/d/Y',$getDate);
	  ...
	  //   $bmi[$i]= round((($res * 703)/($total_inch_height * $total_inch_height)),2);
	
	if($i<10){
	   $line.= "['".$getweek[$i]."',".$bmi[$i]."],";
...
jQuery('#bmi_graph').bind('click.graph',function(){
...
<div class="jqplot" id="chart6"></div>
