...
class Success_journal_model extends Model
{
	...
	function getJournalPost($limit, $perPage, $uid)
	{
		...
		$sql = "(SELECT * FROM `add_journal_entry` WHERE uid='$uid' AND status='2')ORDER BY id DESC limit $limit, $perPage";
	...
	{ 
	    ...
	    $data=array(
					...
					'ownRewardText' =>$this->input->post('ownRewardText'),
					'createdOn' =>$created,
	...
	function orginalMeasure(){
	    ...
	    // $row = array();
	      $uid=$this->session->userdata('id');
          $sql = "select weight,height,goal_weight from `users` where id= '$uid'";
...
/*	
  ...
  function getBonusOptions()
	...
	function getActiveCompliment($uid)
	{
	   $data = array();
       $sql = "(SELECT * FROM `compliment_tracker` WHERE uid='$uid' and active='1' LIMIT 1)";
	   $query = $this->db->query($sql);
        if ($query->num_rows() > 0)
        {
            return $query->result(); 
	...
	function getnewActiveCompliment($getid,$uid)
	{
	   $data = array();
	 
       $sql = "(SELECT * FROM `compliment_tracker` WHERE id ='$getid' and uid='$uid')";
	   $query = $this->db->query($sql);
        if ($query->num_rows() > 0)
        {
            return $query->result(); 
	...
	function getComplimentList($uid)
	{
	   $data = array();
       $sql = "(SELECT * FROM `compliment_tracker` WHERE uid='$uid' ORDER BY id DESC)";
	   $query = $this->db->query($sql);
	...
	function getAscComplimentList($uid)
	{
	   $data = array();
       $sql = "(SELECT * FROM `compliment_tracker` WHERE uid='$uid' AND active='1' ORDER BY id ASC)";
	   $query = $this->db->query($sql);
	...
	function getEditComplimentId($eid)
	{
	   $data = array();
       $sql = "(SELECT * FROM `compliment_tracker` WHERE id='$eid')";
	   $query = $this->db->query($sql);
	...
	function tracker_update($up_id,$up_name,$up_msg)
	{
	  ...
	  //  $created=date("y-m-d");
	    ...
	    $this->db->update('compliment_tracker', $data);	
	}

	
	
	function tracker_delete($did){
             $this->db->where('id', $did);
	         $this->db->delete('compliment_tracker');	
    }
	

	/*function tracker_setActive(){
	        	    $data=array(
		            'active'=>0,
		            );
	...
	function tracker_Active($act,$status){
	...
	function metabolismStartDay(){
	
	    // $row = array();
	      $uid=$this->session->userdata('id');
          $sql = "select metabolism_start_day,createdOn from `user_21day_metabolism` where uid = '$uid'";
          $res = $this->db->query($sql);
	      $row= $res->row_array();
		  return $row;
    }
	
    function initialCtracker(){
	    $data = array();
	      $sql = "SELECT name,blog FROM `initial_ctracker` ORDER by rand() LIMIT 1";
      	  $query = $this->db->query($sql);
		...
		/////////////for plan goals//////////
	function goalPlanResult($id)
	{
	    $query = $this->db->get_where('user_goal_plan', array('user_id' => $id));
         return $query;
    }
	function deleteGoalPlan($id)
	{
        $this->db->where('user_id', $id);
	    $this->db->delete('user_goal_plan');
    }
	function getGoalPlan()
     {
         $goal_plan = $this->db->get('goal_plan');
         return $goal_plan;
     }
	///////////////////end plan goals/////
