...
class Access extends Controller
{
	...
	function __construct()
	{
		...
		$this->load->library(array('Auth', 'form_validation', 'session'));
	...
	{
            $data = array('email'=> $this->uri->segment(3));

            $user = $this->user_model->verifyemail($data);
            ...
            if(empty($user['error']))
            {
                    //$viewVars['error'] = "<b>A confirmation email send to ".$this->uri->segment(3).".</b><br />";
                    $msg = "<b>A confirmation email send to ".$this->uri->segment(3).".</b><br />";
                    $this->session->set_flashdata('viewVars', $msg);
                    //return $this->load->view('/admin/listUser', $viewVars);
