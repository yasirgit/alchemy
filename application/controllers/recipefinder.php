...
class recipefinder extends Controller
{
	...
	{										
		...
		//--------------------add---------------------//
		//$this->viewVars['findrecipe']=$this->db->query($query)->result();
		//--------------------add---------------------//		
	...
	function index()
	{
		...
		$this->viewVars['boxRecipe']=$boxRecipe;
                ...
                $query = "SELECT * FROM	recipes where createdBy ='".$user_id."' ORDER BY rID DESC limit 0,3";
                $latRecipe = $this->db->query($query)->result();
                for($k=0; $k < count($latRecipe); $k++)
		{
			$sql = "SELECT * FROM recipe_images WHERE rID=".$latRecipe[$k]->rID;
			$latRecipe[$k]->imgages = $this->db->query($sql)->row();
		}
		$this->viewVars['latRecipe'] = $latRecipe;
                //print_r($this->viewVars['latRecipe']); exit;
	...
	function xml2array($xml) 
  {
    ...
    */
	...
	function recipeboxmeal()
	{
		...
		$sql="select uj.*,ut.type from user_journal uj, user_times ut where ut.utID=uj.utID and uj.createdBy='".$uid."' AND uj.name IS NOT NULL AND ut.type!='Exercise' order by uj.createdOn desc";				
		$result=$this->db->query($sql)->result();
		
		$total=count($result);
		$total_page=ceil($total/$plimit);
		$result=array_slice($result,($page-1)*$plimit,$plimit);
		
		$journal_item=array();
		$journal_image=array();
		for($i=0;$i<count($result);$i++)	
		{
			$sql="select * from user_journal_items where ujID='".$result[$i]->ujID."'";
			$journal_item[$i]=$this->db->query($sql)->result();
			
			$sql="select * from user_journal_images where utID='".$result[$i]->utID."' AND date='".$result[$i]->date."' AND time='".$result[$i]->time."'";
			$journal_image[$i]=$this->db->query($sql)->result();
		}		
		$data['result']=$result;		
		$data['current_page']=$page;
		$data['total_page']=$total_page;
		$data['journal_image']=$journal_image;
		$data['journal_item']=$journal_item;
		///////////////////////////////////////////////
		$tmp_array	= array();		
		$tmp_array['error_code']	= 0;
		$tmp_array['error_msg']		= "";		
		$tmp_array['display']= $this->load->view('recipefinder/recipeboxmeal',$data,true);		
